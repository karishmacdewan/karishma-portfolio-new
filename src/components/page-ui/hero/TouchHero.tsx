...
export function TouchHero({ reducedMotion = false, onReady }: { reducedMotion?: boolean; onReady?: () => void }) {
    ...
    const promptOpacity = useTransform(scrollYProgress, [0, 0.7, 1], [1, 0.65, 0]);
    ...
    return (
        <section id="home" className={styles.touchHero} data-reduced-motion={shouldReduceMotion} data-theme-scrolling={isThemeScrolling}>
            <div ref={sectionRef} className={styles.touchIdentityChapter}>
                <div className={styles.touchScene}>
                    <motion.div
                        className={styles.typeField}
                        style={shouldReduceMotion ? undefined : { rotate: fieldRotate, scale: fieldScale }}
                        aria-hidden="true"
                    >
                        <motion.span className={styles.fullStack} {...wordAnimation(-80, 0, 0.05)}>
                            strategy
                        </motion.span>
                        <motion.span className={styles.ai} {...wordAnimation(64, -32, 0.15)}>
                            taste
                        </motion.span>
                        <motion.span className={styles.engineer} {...wordAnimation(0, 72, 0.25)}>
                            code
                        </motion.span>
                    </motion.div>

                    <motion.div className={styles.touchCopy} style={shouldReduceMotion ? undefined : { y: identityY }}>
                        <h1
                    ...
                    <motion.div className={styles.touchPrompt} style={shouldReduceMotion ? undefined : { opacity: promptOpacity }}>
