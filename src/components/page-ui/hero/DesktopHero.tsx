...
function ScrollCue({ label, secondary = false }: { label: string; secondary?: boolean }) {
    return (
...
function BarsLayer({ layerIndex, minSpeed, maxSpeed, zIndex, extraHeight }: BarsLayerProps) {
    const bars = useMemo(
        () =>
            Array.from({ length: BAR_COUNT }, (_, index) => {
                const progress = index / (BAR_COUNT - 1);
                const speed = minSpeed + seededFraction(layerIndex * 1000 + index + 1) * (maxSpeed - minSpeed);
            ...
            <div ref={barsChapterRef} className={styles.desktopBarsChapter}>
                <div className={styles.desktopScene}>
                    <div className={styles.desktopCopy}>
                        <h1 className={`${instrumentSerifRegular.className} ${styles.identitySubheading}`} aria-label={HERO_TAGLINE}>
